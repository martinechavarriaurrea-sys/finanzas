...
import { buildQuestionBank } from "./lib/bot_question_bank.mjs";
...
const TOTAL_QUESTIONS = Number(process.env.BOT_STRESS_QUESTIONS || 1000);
...
const RUN_TAG = `${TOTAL_QUESTIONS}x${COMPANIES.length}`;
...
function buildQuestionList(total, payload) {
  ...
  const generated = buildQuestionBank({
    target: total,
    years,
    include_core: true
  });
