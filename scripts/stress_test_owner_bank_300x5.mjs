...
import { buildOwnerQuestionBank } from "./lib/bot_owner_question_bank.mjs";
...
const TOTAL_QUESTIONS = Number(process.env.BOT_OWNER_QUESTIONS || process.env.BOT_STRESS_QUESTIONS || 300);
...
const RUN_TAG = `${TOTAL_QUESTIONS}ownerx${COMPANIES.length}`;
...
function buildQuestionList(total, payload) {
  const years = Array.isArray(payload?.years_selected) && payload.years_selected.length
    ? payload.years_selected
    : [2019, 2020, 2021, 2022, 2023, 2024];
  const generated = buildOwnerQuestionBank({
    target: total,
    years,
    include_core: true
  });
  return generated.map((q) => q.question).slice(0, total);
}
...
async function runCompanyTest(company, totalQuestions) {
  ...
  const questions = buildQuestionList(totalQuestions, payload);
  ...
  for (let i = 0; i < questions.length; i += 1) {
    ...
    let error = "";
    try {
      answer = await callAdvisor(q, payload, localAnswer, chatHistory.slice(-16));
    } catch (e) {
    ...
    if ((i + 1) % 100 === 0) {
      console.log(`[stress] ${company.nit}: ${i + 1}/${questions.length} completadas...`);
    }
  }

  const total = questions.length;
  const failCount = total - okCount;
  const okRate = total ? (okCount / total) * 100 : 0;
  return {
    company,
    ...
    ok_rate_pct: Number(okRate.toFixed(2)),
    latency_ms: {
...
function buildMarkdownReport(results) {
  const lines = [];
  lines.push(`# Prueba masiva del bot (${TOTAL_QUESTIONS} x ${results.length} empresas)`);
  lines.push("");
  lines.push(`- Fecha: ${new Date().toISOString()}`);
  lines.push(`- Tag ejecucion: ${RUN_TAG}`);
  lines.push(`- Total preguntas ejecutadas: ${results.reduce((a, r) => a + r.total_questions, 0)}`);
  lines.push("");

  results.forEach((r, idx) => {
    lines.push(`## Empresa ${idx + 1}: ${r.company.razon_social} (${r.company.nit})`);
    lines.push(`- Anos usados: ${r.years.join(", ")}`);
    lines.push(`- Preguntas: ${r.total_questions}`);
    lines.push(`- Respuestas OK: ${r.ok_count}`);
    lines.push(`- Fallos: ${r.fail_count}`);
    lines.push(`- Tasa OK: ${r.ok_rate_pct}%`);
...
async function main() {
  ...
  for (const company of COMPANIES) {
    const r = await runCompanyTest(company, TOTAL_QUESTIONS);
    results.push(r);
  ...
  const summary = {
    ...
    total_questions_each: TOTAL_QUESTIONS,
    run_tag: RUN_TAG,
    companies: COMPANIES,
    results
  };
  ...
  results.forEach((r) => {
    console.log(`[stress] ${r.company.nit} -> OK ${r.ok_count}/${r.total_questions} (${r.ok_rate_pct}%), p95 ${r.latency_ms.p95}ms`);
  });
